...
deploy_inputs = {
    "img": torch.randn((6,) + data_shape),
    "points": placeholder(
        (6, H, W, 2), torch_native=True, sample=torch.rand((6, H, W, 2))
    ),
}
# data
data_loader = dict(
    ...
    dataset=dict(
        ...
        transforms=[
            dict(type="MultiViewsImgResize", size=(540, 960)),
            dict(type="MultiViewsImgCrop", size=(512, 960)),
...
def process_inputs(infer_inputs, transforms=None):
    ...
    homo = np.load(infer_inputs["homo"])
